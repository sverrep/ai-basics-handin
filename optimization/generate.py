...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        vars = []
        for var in self.crossword.variables:
            tempdomain = self.domains[var].copy()
            for word in tempdomain:
    ...
    def revise(self, x, y):
        ...
        tempxdomain = self.domains[x].copy()
        if overlap:
            for xword in tempxdomain:
                matching = False
            ...
            for neighbor in neighbors:
                    ...
                    for neighborword in self.domains[neighbor]:
                        if (varword[overlap[0]] != neighborword[overlap[1]]): 
                            removed += 1
            domain.append((varword, removed))

        domain = sorted(domain, key = lambda removed: removed[1])
        sorted_domain = []
        for var in domain:
            sorted_domain.append(var[0])
